...
import type { ExposePortResponse, HandlerErrorResponse, ListExposedPortsResponse, Logger, PortInfo, ProxiedErrorResponse, ProxiedSuccessResponse, RequestContext, UnexposePortResponse, ValidatedRequestContext } from '#container/core/types.ts';
import type { PortHandler } from '#container/handlers/port-handler.ts';
import type { PortService } from '#container/services/port-service.ts';
...
describe('PortHandler', () => {
  ...
  beforeEach(async () => {
    ...
    const { PortHandler: PortHandlerClass } = await import('#container/handlers/port-handler.ts');
