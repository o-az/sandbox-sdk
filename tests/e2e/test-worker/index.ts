...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      // Test endpoint: Verify Process methods work in test worker
      // This tests if the Process object's methods (kill, getStatus, getLogs)
      // can be called BEFORE serialization (i.e., within the test worker)
      if (url.pathname === '/api/test/process-methods' && request.method === 'POST') {
        try {
          // Start a long-running process
          const process = await executor.startProcess('sleep 30');

          // Immediately test the methods on the Process object
          const methodTests = {
          ...
          // Test 1: Can we call getStatus()?
          try {
            const status = await process.getStatus();
            methodTests.getStatusWorks = true;
            methodTests.statusValue = status;
          } catch (error) {
            methodTests.getStatusWorks = false;
            methodTests.getStatusError = error instanceof Error ? error.message : String(error);
          }

          // Test 2: Can we call getLogs()?
          try {
            const logs = await process.getLogs();
            methodTests.getLogsWorks = true;
            methodTests.logsValue = {
              stdoutLength: logs.stdout.length,
              stderrLength: logs.stderr.length
            };
          } catch (error) {
            methodTests.getLogsWorks = false;
            methodTests.getLogsError = error instanceof Error ? error.message : String(error);
          }

          // Test 3: Can we call kill()?
          try {
            await process.kill();
            methodTests.killWorks = true;

            // Wait a bit and check status again
            await new Promise(resolve => setTimeout(resolve, 500));
            const statusAfterKill = await process.getStatus();
            methodTests.statusAfterKill = statusAfterKill;
          } catch (error) {
            methodTests.killWorks = false;
            methodTests.killError = error instanceof Error ? error.message : String(error);
          }

          return new Response(JSON.stringify({
            success: true,
            message: 'Process method tests completed',
            results: methodTests
          }), {
            headers: { 'Content-Type': 'application/json' },
          });
